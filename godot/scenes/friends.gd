...
func _ready():
	...
	add_friend_button.connect('pressed', Callable(self, 'on_add_friend_button_pressed'))
...
func on_add_friend_button_pressed():
	...
	var friend_code = await g.summon_text_entry('Friend Code (FC):', '').popup_closed
	if friend_code:
...
func on_get_friends_list(response):
	...
	for friend in response.friends:
		var profile = avatar_profile_scene.instantiate()
		...
		profile.challenge_button.connect('pressed', Callable(self, 'on_challenge_friend').bind(friend))
		if friend.is_online:
...
func on_challenge_friend(friend):
	var confirmed = await g.summon_confirm('Challenge %s to a duel?' % [friend.user_name]).popup_closed
	if confirmed:
