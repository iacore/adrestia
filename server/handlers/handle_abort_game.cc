...
int adrestia_networking::handle_abort_game(const Logger& logger, const json& client_json, json& resp) {
   ...
   *     HANDLER_KEY: <this function>
	 *     "game_uid": The game uid to abort. If no game_uid is specified (or is
	 *     null or "Null"), this endpoint abords matchmaking instead.
   *
   ...
   *                  "Bad game_uid." if game could not be found
	 *                  "Matchmaking aborted." if abortion succeeded
   *
  ...
  string uuid = client_json.at("uuid");
	...
	if (client_json.find("game_uid") != client_json.end() &&
			!client_json.at("game_uid").is_null() &&
			client_json.at("game_uid") != "Null") {
		...
		resp[adrestia_networking::HANDLER_KEY] = client_json[adrestia_networking::HANDLER_KEY];
		resp[adrestia_networking::CODE_KEY] = 200;
		resp[adrestia_networking::MESSAGE_KEY] = "Matchmaking aborted.";
		return 0;
	}
}
